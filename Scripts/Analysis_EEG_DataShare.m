...
% - eeg (EEG)
% - beh (behavioral)
curexperiment.preproc                   = {''}; % 'eeg' or 'beh'
...
curexperiment.analyses                  = {'pow','ga'}; % {'erp','pow','plt','ga','erpga','ana'}; 
...
curexperiment.scriptdir                 = [curexperiment.dirroot,'SouConTACS/SouConTACSFieldtripAnalyses/']; % directory of the script
...
curexperiment.datafolder_output         = [curexperiment.dirroot,curexperiment.name,'/',curexperiment.name,'FieldtripAnalyses']; % location of outputfiles
...
curexperiment.data1.l1.condition2       = str2double(strrep(join(string([combvec(2,[1,2,3,9],[1,2],[4,5])';combvec(3,[1,2,3,9],[1,2],[1,2])'])),' ',''));                                         
...
curexperiment.data1.l1.condition4       = str2double(strrep(join(string(combvec([2,3],[1,2,3,9],[4,5],[1,2,3,4,5,9])')),' ',''));  
...
curexperiment.data2.l1.condition1       = str2double(strrep(join(string([combvec(2,[1,2],[1,2])';combvec(3,[1,2],[4,5])'])),' ',''));                   
curexperiment.data2l1_name{2}           = 'sMiss'; % item hit & source miss
curexperiment.data2.l1.condition2       = str2double(strrep(join(string([combvec(2,[1,2],[4,5])';combvec(3,[1,2],[1,2])'])),' ',''));
...
curexperiment.data2.l1.condition4       = str2double(strrep(join(string(combvec([2,3],[4,5])')),' ',''));
curexperiment.data2l1_name{5}           = 'FA'; % (item) false alarm
curexperiment.data2.l1.condition5       = str2double(strrep(join(string(combvec(1,[1,2],[1,2,3,4,5,9])')),' ',''));
...
curexperiment.data2.l1.condition7       = str2double(strrep(join(string(combvec([1,2,3],[1,2,3,4,5,9],[1,5])')),' ','')); 
curexperiment.data2l1_name{8}           = 'sLC'; % LC source
curexperiment.data2.l1.condition8       = str2double(strrep(join(string(combvec([1,2,3],[1,2,3,4,5,9],[2,3,4])')),' ',''));  
curexperiment.data2l1_name{9}           = 'iHC'; % HC item 
curexperiment.data2.l1.condition9       = str2double(strrep(join(string(combvec([2,3],[1,5],[1,2,3,4,5,9])')),' ','')); 
curexperiment.data2l1_name{10}          = 'iLC'; % LC item
curexperiment.data2.l1.condition10      = str2double(strrep(join(string(combvec([2,3],[2,3,4],[1,2,3,4,5,9])')),' ','')); 
% Combinations
cmbs.c1 = {'Old'};cmbs.c2 = {'iCor'};cmbs.c3 = {'iHC','iLC'};cmbs.c4 = {'sCor','sInc'};cmbs.c5 = {'sHC','sLC'};
[cmbs.c1,cmbs.c2,cmbs.c3,cmbs.c4,cmbs.c5] = ndgrid(cmbs.c1,cmbs.c2,cmbs.c3,cmbs.c4,cmbs.c5);
M = [cmbs.c1(:),cmbs.c2(:),cmbs.c3(:),cmbs.c4(:),cmbs.c5(:)];
...
for i=1:curexperiment.Nsubs
    curexperiment.files(i,:) = {files{contains(files,num2str(i+100))}};
end
clear files df
...
%                                 end
                ...
                % check if we need to do non-ocular artifact rejection or if that is already done for this participant
                if sum(contains({dir(subjectdata.dir_ses).name},'ArtiRemovedFin.mat')) ~= length(curexperiment.dataset_name)
                    % load in the outputfile
                    load(curexperiment.outputfile)
                    % check if we need to load in data
                    if exist('event','var')~=1
                    ...
                    for l=1:curexperiment.(['data',num2str(d)]).levels 
                        ...
                        try
                            ...
                            if d==2 && l==1 
                                ...
                                cur_diffs = {'iHit-CR','iHC-iLC'};
                                for dif = 1:length(cur_diffs)
                                    cur_diff = ismember(curconname,split(cur_diffs{dif},'-'));
                                    limt = [1.2, .04];
...
if any(contains(curexperiment.analyses,'plt'))
    ...
    for ses=1
        % loop over the datasets
        for d=1:length(curexperiment.dataset_name)-1
            % loop over all levels of processing
            for l=1:curexperiment.(['data' num2str(d)]).levels-1
                ...
                if ~isempty(TFfiles_lev)
                    ...
                    for c=1:length(curconname)
                        % create an array to hold the inputfiles for the different conditions
                        curmatfiles = TFfiles_lev(contains(TFfiles_lev,curexperiment.(['data' num2str(d) 'l' num2str(l) '_name']){c}));
                        % exclude participants
                        if c==3 %iHit
                            excl_ppn = {}%{'135','114','116','151','122','148'};
                    ...
                    try
                        % CALCULATE THE DIFFERENCE
                        if d==2 && l==1 
                            %cur_diff = [3 6]; limt = [.6, .005]; % iHit-CR
                            %cur_diff = [1 2]; limt = [.4, .009]; % sHit-sMiss
                            %cur_diff = [9 10]; limt = [.6, .015]; % iHC-iLC
                            %cur_diff = [7 8]; limt = [.4, .006]; % sHC-sLC
                            cur_diff = [6 5]; limt = [.4, .009]; % CR-FA
                        else 
                        end
                        limt_t = [2 2]; % limits for the t-values
                        % store only the datasets that are currently needed
